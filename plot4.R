...
plot4 <- function(filename = "../household_power_consumption.txt") {
    ...
    twodays <- c("2007-02-01","2007-02-022") # 2 days date we are interested on
    ...
    ## Plot 2
    Voltage <- as.numeric(elepow$Voltage)
    ...
    ## Plot 4
    GRP <- as.numeric(elepow$Global_reactive_power)
    ...
    png(filename = "plot4.png", width = 480, height = 480, units = "px",
        bg = "white")
    par(mfrow = c(2,2))
    plot(WD, GAP, ylab = "Global Active Power", xlab = "", type = "l")
    plot(WD, Voltage, ylab = "Voltage", xlab = "datetime", type = "l")
    ...
    plot(WD, GRP, ylab = "Global_reactive_power", xlab = "datetime", type = "l")
